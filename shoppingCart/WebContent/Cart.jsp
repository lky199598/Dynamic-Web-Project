	...
	<div id="shopping">
		<table>
			...
			<tr name="products" id="product_id_1">
				...
				<td class="delete"><a
					href="Servlet/CartServlet?action=BuyShopping&Buy_id=<%=item.getId()%>&name=<%=item.getName()%>&city=<%=item.getCity()%>&picture=<%=item.getPicture()%>&price=<%=item.getPrice()%>&buy_number=<%=c.getCart_number()%>"
